...
from yolov3_tf2 import __path__ as local_path
...
class YoloDetectorNode():
    def __init__(self,
                 ...
                 tiny = False):
        self.real_path = '/home/melodic/flow_ws/src/yolov3-tf2'
        ...
        if tiny == True:
            ...
            self.yolo.load_weights(self.real_path+'/checkpoints/yolov3-tiny.tf').expect_partial()
        else:
            self.yolo = YoloV3()
            self.yolo.load_weights(self.real_path+'/checkpoints/yolov3.tf').expect_partial()
        print('Weights loaded!')
        self.class_names = [c.strip() for c in open(self.real_path+'/data/coco.names').readlines()]
        print("Classes loaded!")
        rospy.wait_for_message(image_topic, CompressedImage)
        self.pcl_msg = rospy.wait_for_message(point_cloud_topic, PointCloud2)
        ...
        rospy.Subscriber(point_cloud_topic, PointCloud2,self.pcl_topic_callback)
